...
local function find_existing_claude_terminal()
  ...
  for _, buf in ipairs(buffers) do
    if vim.api.nvim_buf_is_valid(buf) and vim.api.nvim_buf_get_option(buf, "buftype") == "terminal" then
      -- Check if this is a Cursor Code terminal by examining the buffer name or terminal job
      ...
      if buf_name:match("claude") then
        ...
        for _, win in ipairs(windows) do
          if vim.api.nvim_win_get_buf(win) == buf then
            logger.debug("terminal", "Found existing Cursor terminal in buffer", buf, "window", win)
            return buf, win
          end
        end
      end
...
function M.open(cmd_string, env_table, effective_config, focus)
  ...
  if is_valid() then
    ...
    -- Check if there's an existing Cursor terminal we lost track of
    ...
    if existing_buf and existing_win then
      ...
      logger.debug("terminal", "Recovered existing Cursor terminal")
      if focus then
      ...
      if not open_terminal(cmd_string, env_table, effective_config, focus) then
        vim.notify("Failed to open Cursor terminal using native fallback.", vim.log.levels.ERROR)
      end
    end
  end
end
    ...
    if has_buffer then
      ...
      if existing_buf and existing_win then
        ...
        if not open_terminal(cmd_string, env_table, effective_config) then
          vim.notify("Failed to open Cursor terminal using native fallback (simple_toggle).", vim.log.levels.ERROR)
        end
      end
    end
  end
...
function M.focus_toggle(cmd_string, env_table, effective_config)
  ...
  if has_buffer then
    ...
    if existing_buf and existing_win then
      ...
      -- No existing terminal found, create a new one
      if not open_terminal(cmd_string, env_table, effective_config) then
        vim.notify("Failed to open Cursor terminal using native fallback (focus_toggle).", vim.log.levels.ERROR)
      end
    end
  end
end
