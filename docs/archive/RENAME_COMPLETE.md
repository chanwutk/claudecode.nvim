...
1. **TROUBLESHOOTING.md** - Comprehensive guide for the "module 'cursor-cli' not found" error
...
- Added critical `name = "cursor-cli"` requirement
- Added troubleshooting section
- Updated all installation examples

## For Your Users

### To Fix the "module 'cursor-cli' not found" Error

Your user needs to update their Neovim configuration:

#### Current Issue
The error shows that Neovim can't find the `cursor-cli` module. This is because:
1. The repository name changed
2. They might be using the old repository name
3. They might be missing `name = "cursor-cli"` in their lazy.nvim config
...
-- Change from old (or add name if missing):
{
  ...
  -- or missing: name = "cursor-cli"
}

-- To new:
{
  "chanwutk/cursor-cli.nvim",  -- ✅ Correct repository name
  name = "cursor-cli",           -- ✅ Critical: set module name
...
✅ Module structure intact (lua/cursor-cli/)
✅ Plugin loader present (plugin/cursor-cli.lua)
...
- **Module**: `cursor-cli`
- **Commands**: `CursorCLI*`
- **Require**: `require("cursor-cli")`

### Critical for lazy.nvim Users
```lua
{
  "chanwutk/cursor-cli.nvim",
  name = "cursor-cli",  -- ⚠️ This line is REQUIRED!
}
```

## No Breaking Changes

- Same Lua module name: `cursor-cli`
- Same commands: `CursorCLI*`
- Same functionality
- Same API
...
2. Add `name = "cursor-cli"` if using lazy.nvim
