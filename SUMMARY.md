...
### Basic Commands
```vim
:ClaudeCode              " Open cursor terminal
:ClaudeCodeAdd %         " Send current file (@filename)
:ClaudeCodeAdd % 10 20   " Send file with lines (@filename:10-20)
:ClaudeCodeSend          " Send visual selection (in visual mode)
```
...
3. Run `:ClaudeCode` (should open cursor)
4. Run `:ClaudeCodeAdd %` (should type `@filename`)
